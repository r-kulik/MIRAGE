...
df = pd.read_pickle("only_q.pkl")


indexes_counted = [int(x.split("_")[0]) for x in os.listdir("answers_base")]
...
async def worker(queue: asyncio.Queue, key_index: int, executor: ThreadPoolExecutor):
    ...
    while True:
        ...
        try:
            ...
            # Save periodically
            with open(
                f"answers_base/{row_index}_answer.json", "w", encoding="utf-8"
            ) as file:
