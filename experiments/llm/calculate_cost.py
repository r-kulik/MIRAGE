...
df = pd.read_pickle(r"E:/final.pkl")


indexes_counted = [int(x.split("_")[0]) for x in os.listdir("answers")]
# print(indexes_counted)
df = df[~df.index.isin(indexes_counted)]
PROGREESS_BAR = tqdm(total=df.shape[0])

# df1 = df[df['dataset']=='bench']
# df2 = df[df['dataset']=='cuad']

# t1 = df1['texts'].apply(lambda x: sum([len(i) / 3 for i in x]))
# t2 = df2['texts'].apply(lambda x: sum([len(i.split()) * 1.2 for i in x]))


# t1.sum() * (0.29 * 0.75) / 1e6 * 82.52


# t2.sum() * (0.29 * 0.75) / 1e6 * 82.52


# question = df1.iloc[1920]
# chunks = question.texts
# q = question.question
# r = question.response


# df1[df1.file.str.contains('32')]

# df

# sum([len(i) for i in chunks])


# sum([len(i.split()) for i in chunks])
...
#         continue
#     answer, generation_time = do_safe_request(row)
#     answers.append({
...
#     })
#     if len(answers) >= SAVE_AMOUNT:
#         with open(f'answers\\{START_INDEX}_{index}_answers.json', 'w', encoding='utf-8') as file:
#             file.write(json.dumps(answers, ensure_ascii=False, indent=4))
#         answers = []
#         START_INDEX = index + 1
...
async def worker(queue: asyncio.Queue, key_index: int, executor: ThreadPoolExecutor):
    ...
    while True:
        ...
        try:
            ...
            # Save periodically
            with open(
                f"answers/{row_index}_answer.json", "w", encoding="utf-8"
            ) as file:
