...
K1_DIAPASON = [1, 1.2, 1.5]
C_DIAPASON = [1, 2, 4, 8]

df = pd.read_pickle("E:\\fulltextsearch_final_res.pkl")
df = df[df.c.isin(C_DIAPASON) & df.k1.isin(K1_DIAPASON)]

indexes_counted = [int(x.split("_")[0]) for x in os.listdir("answers_fulltext")]
    ...
    while True:
        try:
            ...
            answer = llm.do_request(
                query=row["question_text"], chunks=chunks_to_add, lang=language
            )
            print(answer)
            end_time = time.time()
            time.sleep(2)
...
async def worker(queue: asyncio.Queue, key_index: int, executor: ThreadPoolExecutor):
    ...
    while True:
        ...
        try:
            ...
            # Save periodically
            with open(
                f"answers_fulltext/{row_index}_answer.json", "w", encoding="utf-8"
            ) as file:
