...
class MIRAGE:
    def __init__(self, index: MirageIndex, inference = MirageInference, embedder = Embedder) -> None:
    ...
    def query(self, query_text):
        return self.inference.llm_adapter.do_request(
            ...
            chunk_storage=self.index.chunk_storage,
            indexes=[
                    result.chunk_storage_key for result in self.inference.reranker(
                        fulltext_search_results=self.index.chunk_storage.query(
                            query=query_text
                        )
                        if self.inference.quorum is None else self.inference.quorum.query(
                            text=query_text
                        ),
