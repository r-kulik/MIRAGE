...
class WeightedProductReranker(Reranker):
    ...
    def __init__(
        self,
        fulltext_power: float = 1,
        vector_power: float = 1,
    ...
    def __call__(
        ...
        if self.minmax_normalize:
            fulltext_search_results = Reranker.minmax_score_normalization(
                fulltext_search_results
            )
            vector_search_results = Reranker.minmax_score_normalization(
        ...
        for result in vector_search_results:
            if result.chunk_storage_key in combined_results:
                raise ValueError(
                    "There is an attempt to add a single chunk twice in rank fusion"
                )
            combined_results[result.chunk_storage_key] = ScorePair(
                fulltext_score=0, vector_score=result.score
            )
        for result in fulltext_search_results:
            ...
            combined_results[result.chunk_storage_key] = ScorePair(
                fulltext_score=result.score, vector_score=vector_score
            )
        return list(
            sorted(
                [
                    QueryResult(
                        score=(
                            score_pair.fulltext_score**self.fulltext_power
                            * score_pair.vector_score**self.vector_power
                        ),
