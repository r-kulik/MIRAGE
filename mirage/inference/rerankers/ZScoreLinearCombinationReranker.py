...
class ZScoreLinearCombinationReranker(LinearCombinationReranker):
    ...
    def __init__(self, fulltext_score_weight, vector_score_weight):
        super().__init__(fulltext_score_weight=fulltext_score_weight, vector_score_weight=vector_score_weight)
    
    def __call__(self, fulltext_search_results: List[QueryResult], vector_search_results: List[QueryResult]) -> List[QueryResult]:
        fulltext_search_results_normalized = Reranker.score_standartization(search_results=fulltext_search_results)
        vector_search_results_normalized = Reranker.score_standartization(search_results=vector_search_results)
        return super().__call__(fulltext_search_results=fulltext_search_results_normalized, 
