    ...
    def __call__(
        ...
        for j, result in enumerate(fulltext_search_results):
            if result.chunk_storage_key not in results:
                ...
                results[result.chunk_storage_key] = RankPair(
                    fulltext_rank=j, vector_rank=len(vector_search_results) - 1
                )
        return list(
            sorted(
                (
                    QueryResult(
                        ...
                        vector=None,
                        score=(1 / (rank_pair.vector_rank + self.c))
                        + (1 / (rank_pair.fulltext_rank + self.c)),
                    )
