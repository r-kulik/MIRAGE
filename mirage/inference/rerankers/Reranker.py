...
class Reranker(ABC):
    ...
    @staticmethod
    def minmax_score_normalization(
        search_results: List[QueryResult],
    ) -> List[QueryResult]:
        if not search_results:
            return []
        minmax_norm: Callable[[float, float, float], float] = lambda x, minx, maxx: (
            x - minx
        ) / (maxx - minx)
        ...
        return [
            QueryResult(
                score=(
                    ...
                    else minmax_norm(result.score, min_score, max_score)
                ),
    ...
    def score_standartization(search_results: List[QueryResult]) -> List[QueryResult]:
        ...
        vector_search_results_normalized = [
            QueryResult(
                score=(
                    ((query_result.score - mean_score) / std_score)
                    if std_score != 0
                    else 0
                ),
