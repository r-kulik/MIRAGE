...
class RusVectoresQuorum:
    ...
    rus_vectores_POS_list: list[str] = [
        "ADJ",
        "VERB",
        "NUM",
        "ADV",
    ...
    def __init__(
        ...
        if type(POS_thresholds) == list:
            self.POS_thresholds = {
                x: global_similarity_threshold for x in POS_thresholds
            }
        else:
            self.POS_thresholds = POS_thresholds
        if not all(
            x in RusVectoresQuorum.rus_vectores_POS_list for x in self.POS_thresholds
        ):
            raise ValueError(
                "You have provided invalid POS tag in the POS thresholds argument"
            )
        # ----------------------------------------------------------------------
        ...
        self.relative_cosine_similarity_strategy: bool = (
            relative_cosine_similarity_strategy
        )
        self.TOP_N = 10
        self.RCS_THRESHOLD = 0.11
        # ------------ loading the Word2Vec Model from the file
        self.word_vectors: KeyedVectors = KeyedVectors.load_word2vec_format(
        ...
        if self.visualize:
            ...
            print(
                f'word "быть" is presented in word vectors: {"быть_VERB" in self.word_vectors}'
            )
            print(
                f'set of POS in w2v model: {set([x.split("_")[1] for x in self.word_vectors.key_to_index])}'
            )
    ...
    def get_relative_cosine_similarity_synonyms(self, lemma: str):
        ...
        filtered_synonyms = [
            (syn, sim)
            for syn, sim in candidate_synonyms
    ...
    def _get_synonyms(self, word: str, pos: str) -> List[Tuple[str, float]]:
        ...
        if self.relative_cosine_similarity_strategy:
            synonyms = self.get_relative_cosine_similarity_synonyms(lemma)
        else:
            try:
    ...
    def query(self, text: str) -> List[ChunkNote]:
        ...
        while heap and combination_count < self.max_combinations:
            ...
            if self.visualize:
                print(f"SEARCH QUERY with sim {current_sim}: {modified}")
            chunk_results = self.chunk_storage.query(modified)
            CALLED += 1
            logs.append(replacements)
            # --------------------------------------------------------
            ...
            for r_idx, (word_idx, syns) in enumerate(replaceable):
                ...
                # it is possible
                    ...
                    if current_syn_idx + 1 < len(syns):
                        ...
                        new_sim = (
                            current_sim
                            / syns[current_syn_idx][1]
                            * syns[current_syn_idx + 1][1]
                        )
                        heappush(heap, (-new_sim, new_sim, new_replacements))
                # --------------------------------------------------------
        ...
        if self.inv_mode:
            return list(sorted(results, key=lambda x: x.score, reverse=True)), CALLED, logs
        return list(sorted(results, key=lambda x: x.score, reverse=True))
