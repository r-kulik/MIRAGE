...
class NatashaSentenceChunking(ChunkingAlgorithm):
    ...
    def chunk_a_document(self, raw_document_index) -> int:
        ...
        for i in range(
            0, len(sentences), self.sentences_in_chunk - self.overlap_size_in_sentences
        ):
            self.chunk_storage.add_chunk(
                text=" ".join(
                    map(lambda x: x.text, sentences[i : i + self.sentences_in_chunk])
                ),
