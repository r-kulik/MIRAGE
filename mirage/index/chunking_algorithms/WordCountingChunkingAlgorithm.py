...
class WordCountingChunkingAlgorithm(ChunkingAlgorithm):
    ...
    def __init__(
        ...
        if type(words_amount) != int or words_amount <= 0:
            raise ValueError(
                f"Incorrect words_amount = {words_amount} was passed in the initialization of the WordCountingChunkingAlgorithm"
            )
        if not (0 <= overlap <= 1):
            raise ValueError(
                f"Incorrect overlap = {overlap} was passed in the initialization of the WordCountingChunkingAlgorithm. It should be between 0 and 1."
            )
    ...
    def chunk_a_document(self, raw_document_index: str) -> int:
        ...
        for i in range(0, len(words), self.words_amount - overlap_size):
            chunk_text = " ".join(words[i : min(i + self.words_amount, len(words))])
