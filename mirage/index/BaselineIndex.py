...
class BaselineIndex(MirageIndex):
    ...
    def __init__(
        ...
        raw_storage = MirageIndex._moduleInstanceRedefining(
            raw_storage, RawStorage, FolderRawStorage(data_folder)
        )
        chunk_storage = MirageIndex._moduleInstanceRedefining(
            chunk_storage, ChunkStorage, RAMChunkStorage()
        )
        chunking_algorithm = MirageIndex._moduleInstanceRedefining(
            chunking_algorithm,
            ChunkingAlgorithm,
            WordCountingChunkingAlgorithm(
                raw_storage, chunk_storage, words_amount=words_amount_in_chunk
            ),
        )
        embedder = MirageIndex._moduleInstanceRedefining(
            embedder, Embedder, BowEmbedder(normalizer=normalizer)
        )
