...
from mirage.index.chunking_algorithms.NatashaSentenсeChunking import (
...
def get_name(idx, c):
    ch = (
        "W"
        if c["ChunkingAlgorithm"]["method"] == "WordCountingChunkingAlgorithm"
        else "S"
    )
...
def generate_index(combination: Dict, filepath_prefix: str) -> None:
    ...
    match combination["ChunkingAlgorithm"]["method"]:
        case "WordCountingChunkingAlgorithm":
            chunking_algorithm = WordCountingChunkingAlgorithm(
                ...
                **combination["ChunkingAlgorithm"]["params"],
            )
        case "SentenceChunkingAlgorithm":
            chunking_algorithm = NatashaSentenceChunking(
                raw_storage=raw_storage,
                chunk_storage=chunk_storage,
                **combination["ChunkingAlgorithm"]["params"],
            )
    ...
    embedder = HuggingFaceEmbedder(
        model_name=combination["Embedder"]["params"]["model"]
    )
    ...
    embedder.convert_chunks_to_vector_index(
        chunk_storage=chunk_storage, vector_index=l2_index, visualize=True
    )
...
for indx in range(START_COMBINATION, END_COMBINATION):
    ...
    generate_index(combination, get_name(indx, combination))
