...
def test_mirage_index_initialization(index_class, temp_data_folder):
    index = index_class(
        data_folder=temp_data_folder, words_amount_in_chunk=3, visualize=True
    )
...
def test_query(index_class, temp_data_folder):
    ...
    assert any(["test" in result.text for result in results])
...
def test_custom_parameters(index_class, temp_data_folder):
    ...
    custom_chunking_algorithm = WordCountingChunkingAlgorithm(
        custom_raw_storage, custom_chunk_storage, words_amount=3
    )
